...
export default function Contacts() {
  ...
  async function onSubmit({ name, email, subject, message }: EmailFormType) {
    try {
      ...
      const contact: Contact = {
        ...
        honeypot: '',
        replyTo: '@',
        accessKey: process.env.staticFormsPrivateKey as string,
      }
  ...
  return (
    <section
      id="contacts"
      className={`bg-stone-100 w-full flex flex-col items-center gap-20 text-stone-800 font-light text-2xl max-xl:text-3xl py-20 px-20 max-md:px-4 max-xl:py-10 max-lg:px-10 max-xl:px-14 max-sm:pb-0  ${styles.contacts}`}
    >
      <H3 text="CONTATO" />
      <div className="flex flex-row max-xl:flex-col gap-12 w-full max-w-7xl">
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="w-3/5 max-xl:w-4/5 max-lg:w-full flex flex-col max-xl:self-center max-xl:order-1 gap-7 px-16 max-sm:px-1 py-16 lg:pt-0 rounded-lg"
        >
          <label htmlFor="name">
            ...
            />
            <small className="text-base max-md:text-base">
          ...
          <button className="flex gap-2 justify-center items-center rounded-md px-20 py-5 mt-7 max-xl:px-12 text-stone-50 text-lg bg-slate-900 font-semibold shadow-lg hover:-translate-y-0.5 transition-all duration-300 ease-out">
            <CircleNotch
              className={`animate-spin ${isFormSubmitting ? '' : 'hidden'}`}
            />
            <span>ENVIAR</span>
          </button>
        </form>
        <div className="w-2/5 max-xl:w-full flex flex-col gap-10 lg-mt-1">
          <p>
            Sinta-se à vontade para preencher o formulário de contato{' '}
            <span className="max-xl:hidden">ao lado</span>
            <span className="xl:hidden">abaixo</span>. Estou ansioso para
            receber suas mensagens e responder o mais rápido possível. Obrigado
            por considerar entrar em contato!
          </p>
          <nav className="flex flex-col max-xl:items-center  gap-2 text-xl max-xl:text-2xl">
            <div className="max-xl:w-[33rem] max-sm:w-full max-md:text-xl">
              ...
              <Link href="https://github.com/isaqueback" target="_blank">
                isaqueback
              </Link>
            </div>
            <div className="max-xl:w-[33rem] max-sm:w-full max-md:text-xl">
            ...
            <div className="max-xl:w-[33rem] max-sm:w-full max-md:text-xl">
              ...
              <Link href="mailto:isa.quecosta00@gmail.com" target="_blank">
                isa.quecosta00@gmail.com
              </Link>
            </div>
          </nav>
        </div>
      ...
      <Dialog open={isModalOpen} onClose={handleCloseModal}>
        <DialogContent
          className={`bg-stone-100 flex gap-4 ${
            response.type === 'success' ? 'text-green-600' : 'text-red-500'
          }`}
        >
        ...
        <DialogActions className="bg-stone-700">
          <Button onClick={handleCloseModal} className="text-stone-50">
            Fechar
          </Button>
        </DialogActions>
      </Dialog>
